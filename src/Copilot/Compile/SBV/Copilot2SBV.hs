...
c2sExpr :: 
  C.Id -> MetaTable -> (forall e . C.Expr e => e a) -> S.SBVCodeGen (S.SBV a)
c2sExpr id meta e = 
...
newtype C2SExpr a = C2SExpr
  { c2sExpr_ :: Env -> MetaTable -> QueueInputs -> S.SBVCodeGen (S.SBV a) }
...
instance C.Expr C2SExpr where
  const t x = C2SExpr $ \ _ _ _ -> 
    case W.symWordInst t of W.SymWordInst -> return $ S.literal x
  ----------------------------------------------------

  drop t i id = C2SExpr $ \ _ meta inputs ->
    -- let Just strmInfo = M.lookup id (streamInfoMap meta) in
    -- drop1 t strmInfo
    -- where
    ...
    drop1 t1
      ...
      do W.SymWordInst <- return $ W.symWordInst t2
         W.HasSignAndSizeInst <- return $ W.hasSignAndSizeInst t2
         val <- undefined --Q.lookahead i que
         return $ coerce (cong p) val

  ----------------------------------------------------

  local t1 _ name e1 e2 = C2SExpr $ \ env meta inputs -> do
    e1'  <- c2sExpr_ e1 env meta inputs
    let env' = M.insert name (Local e1' t1) env 
    c2sExpr_ e2 env' meta inputs

  ----------------------------------------------------

  var t1 name = C2SExpr $ \ env _ _ ->
  ...
  extern t name = C2SExpr $ \ _ meta _ -> 
  ...
  op1 op e = C2SExpr $ \ env meta inputs -> do
    res1 <- c2sExpr_ e env meta inputs
    c2sOp1 op res1

  ----------------------------------------------------

  op2 op e1 e2 = C2SExpr $ \ env meta inputs -> do
    res1 <- c2sExpr_ e1 env meta inputs
    res2 <- c2sExpr_ e2 env meta inputs
    c2sOp2 op res1 res2 

  ----------------------------------------------------

  op3 op e1 e2 e3 = C2SExpr $ \ env meta inputs -> do
    res1 <- c2sExpr_ e1 env meta inputs
    res2 <- c2sExpr_ e2 env meta inputs
    res3 <- c2sExpr_ e3 env meta inputs
